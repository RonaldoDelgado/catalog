...
import { downloadAllProductQRCodes } from '@/utils/qrGenerator';
...
export default function ProductManager() {
  ...
  const handleDownloadAllQRCodes = async () => {
    ...
    const confirmed = confirm(
      `¿Deseas descargar ${products.length} códigos QR? Esto puede tomar unos minutos.`
    );

    if (!confirmed) return;

    setIsDownloadingQR(true);
    setDownloadProgress({ current: 0, total: products.length });

    try {
      await downloadAllProductQRCodes(products, (current, total) => {
        setDownloadProgress({ current, total });
      });
      alert('¡Códigos QR descargados exitosamente!');
    } catch (error) {
      console.error('Error descargando QR codes:', error);
      alert('Error al descargar los códigos QR. Por favor, intenta de nuevo.');
    } finally {
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-3">
          ...
          <button
            ...
            disabled={isDownloadingQR || products.length === 0}
            className="bg-purple-600 text-white px-4 py-2 rounded-md hover:bg-purple-700 transition-colors flex items-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isDownloadingQR ? (
              <>
